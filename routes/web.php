...
Route::middleware(['auth'])->prefix('rekrutmen')->name('rekrutmen.')->group(function () {
    ...
    Route::get('calendar', [RecruitmentDashboardController::class , 'calendarPage'])->name('calendar');
            ...
            Route::post('fpk/{fpk}/approve-hr-manager', [FpkController::class, 'approveHrManager'])->name('fpk.approveHrManager');
            Route::post('fpk/{fpk}/request-revision', [FpkController::class, 'requestRevision'])->name('fpk.requestRevision');
...
// --- KOMPETENSI    // Monitoring Kompetensi
    Route::get('/kompetensi/monitoring', [App\Http\Controllers\KompetensiController::class, 'index'])->name('kompetensi.monitoring');
    Route::post('/kompetensi/sync', [App\Http\Controllers\KompetensiController::class, 'sync'])->name('kompetensi.sync');
    Route::get('/kompetensi/export', [App\Http\Controllers\KompetensiController::class, 'export'])->name('kompetensi.export');
...
Route::middleware(['auth', 'role:admin|superadmin'])->prefix('organization')->name('organization.')->group(function () {
    ...
    Route::post('company/{company}/upload-struktur', [\App\Http\Controllers\CompanyController::class, 'uploadStruktur'])
        ->name('company.upload-struktur');
    Route::delete('company/{company}/delete-struktur', [\App\Http\Controllers\CompanyController::class, 'deleteStruktur'])
        ->name('company.delete-struktur');
