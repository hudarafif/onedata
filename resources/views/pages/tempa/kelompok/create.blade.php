...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Tambah Kelompok TEMPA</h1>
            <p class="mt-2 text-gray-600 dark:text-gray-400">
                    Form untuk menambahkan kelompok TEMPA baru
            </p>
        </div>
    ...
    <form action="{{ route('tempa.kelompok.store') }}" method="POST" class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        @csrf
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <select name="tempat" id="tempat" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                           :class="isOptionSelected && 'text-gray-800 dark:text-white/90'" @change="isOptionSelected = true" required>
                    <option value="pusat" {{ old('tempat') == 'pusat' ? 'selected' : '' }}>Pusat</option>
                    <option value="cabang" {{ old('tempat') == 'cabang' ? 'selected' : '' }}>Cabang</option>
                </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const keteranganCabangField = document.getElementById('keterangan_cabang_field');
