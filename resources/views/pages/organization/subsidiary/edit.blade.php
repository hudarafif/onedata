...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <nav class="mb-6">
        <ol class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <li class="flex items-center gap-2">
                ...
                <span class="text-gray-900 dark:text-white">Edit Anak Perusahaan</span>
            </li>
        </ol>
    </nav>
    ...
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
            Edit Anak Perusahaan
        </h1>
        <p class="mt-1 text-gray-600 dark:text-gray-400">
            Perbarui data anak perusahaan <strong>{{ $subsidiary->name }}</strong>
        </p>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
        <form action="{{ route('organization.subsidiary.update', $subsidiary->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                <x-searchable-select
                    ...
                    :options="$parentCompanies->map(fn($c) => [
                        'id' => $c->id,
                        'name' => $c->name,
                        'detail' => $c->holding ? '(' . $c->holding->name . ')' : ''
                    ])"
                    detail-key="detail"
                    x-data="{ parent_id: '{{ old('parent_id', $subsidiary->parent_id) }}' }"
                    x-model="parent_id"
                    required
                />
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Pilih perusahaan induk. Anak perusahaan akan tergabung dalam Holding yang sama dengan induknya.
                </p>
            </div>
            ...
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Nama Anak Perusahaan <span class="text-red-500">*</span>
                </label>
                <input
                    type="text"
                    id="name"
                    ...
                    placeholder="Contoh: PT. Anak Perusahaan Jaya"
                    required
                />
                @error('name')
