...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-gray-600 dark:text-gray-400">
                Kelola data holding (Yayasan / Manufaktur)
            </p>
        </div>

        <a href="{{ route('organization.holding.create') }}"
    ...
    <div x-data="holdingTable()" class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition">
                            ...
                            <td class="px-5 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a :href="'{{ url('organization/holding') }}/' + row.id" class="inline-flex items-center justify-center rounded-lg bg-blue-50 p-2 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/40 transition mr-1" title="Lihat">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <a :href="'{{ url('organization/holding') }}/' + row.id + '/edit'"
                                    ...
                                    <form :action="'{{ url('organization/holding') }}/' + row.id" method="POST"
                                        @submit.prevent="if(confirm('Apakah Anda yakin ingin menghapus holding ini?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    <template x-if="filtered.length === 0">
                        <tr>
                            <td colspan="4" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                                Belum ada data holding ditemukan.
                            </td>
                        </tr>
                    </template>
                </tbody>
        ...
        get filtered() {
            ...
            if (this.search) {
                ...
                filteredData = filteredData.filter(d =>
                    d.name.toLowerCase().includes(q) || (d.type && d.type.toLowerCase().includes(q))
                );
            }
